...
def main():
    ...
    def mouse_callback(event, x, y, flags, param):
        ...
        if event == cv2.EVENT_LBUTTONDOWN or (event == cv2.EVENT_MOUSEMOVE and mouse_dragging):
            ...
            if event == cv2.EVENT_LBUTTONDOWN:
                print(
                    f"Clicked at pixel ({x}, {y}) -> "
                    f"Set angles: yaw={yaw:.1f}°, pitch={pitch:.1f}°, roll={roll:.1f}°"
                )
    ...
    processor = Equirectangular2PinholeProcessor(output_width=1920, output_height=1080, fov=90)
        ...
        while camera.is_connected:
            ...
            if frame is not None:
                ...
                if isinstance(projected, dict):
                    # Skip this frame if processor returns unexpected format
                    continue

                # Back-project center of projected image to equirectangular coordinates
                ...
                cv2.line(
                    projected,
                    (center_x_proj - 15, center_y_proj),
                    (center_x_proj + 15, center_y_proj),
                    (0, 255, 255),
                    3,
                )
                cv2.line(
                    projected,
                    (center_x_proj, center_y_proj - 15),
                    (center_x_proj, center_y_proj + 15),
                    (0, 255, 255),
                    3,
                )
                ...
                if frame_count % 30 == 0:  # Print every 30 frames (~1 second at 30fps)
                    print(
                        f"Center projects to equirectangular coords: ({eq_x}, {eq_y}) | "
                        f"Angles: pitch={pitch:.1f}°, yaw={yaw:.1f}°"
                    )
