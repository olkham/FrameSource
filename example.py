...
def test_audio_spectrogram(source=None, **kwargs):
    ...
    if frame_source.is_connected:
        print("Audio spectrogram params:")
        ...
        while frame_source.is_connected:
            ...
            if key == 27:  # ESC key to quit
                ...
                if hasattr(frame_source, "get_contrast_method") and hasattr(
                    frame_source, "set_contrast_method"
                ):
                ...
                if hasattr(frame_source, "get_gamma_correction") and hasattr(
                    frame_source, "set_gamma_correction"
                ):
                    ...
                    contrast = "more contrast" if new_gamma < 1.0 else "less contrast"
                    print(f"Gamma correction: {new_gamma:.2f} ({contrast})")
                ...
                if hasattr(frame_source, "get_noise_floor") and hasattr(
                    frame_source, "set_noise_floor"
                ):
                ...
                if hasattr(frame_source, "get_percentile_range") and hasattr(
                    frame_source, "set_percentile_range"
                ):
...
if __name__ == "__main__":
    # test_audio_spectrogram(source=None, threaded=True, n_mels=256, window_duration=3.0, freq_range=(20, 20000),  # noqa: E501
    #                        sample_rate=44100, db_range=(-60, 0), contrast_method='adaptive',
    #                        gamma_correction=2.0, noise_floor=-45, percentile_range=(0, 100), colormap=cv2.COLORMAP_INFERNO)  # noqa: E501
    ...
    # processor = RealsenseDepthProcessor(output_format=RealsenseProcessingOutput.ALIGNED_SIDE_BY_SIDE)  # noqa: E501
    # camera.attach_processor(processor)
    # test_camera(camera)
    ...
    # test_camera('basler')
    # test_camera('basler')
    # test_camera('webcam', source=0, threaded=True, width=1920, height=1080, fps=30)   # standard 1080p webcam  # noqa: E501
    # test_camera('webcam', source=0, threaded=True, width=2880, height=1440, fps=30)   # insta360 x5 webcam mode settings  # noqa: E501
    # test_camera('video_file', source="media/geti_demo.mp4", loop=True)
    # test_camera('ipcam', source="rtsp://192.168.1.153:554/h264Preview_01_sub", username="admin", password="password")  # noqa: E501
    # test_camera('folder', source="media/image_seq", sort_by='date', fps=30, real_time=True, loop=True)  # noqa: E501
    # test_camera('screen', x=100, y=100, w=800, h=600, fps=30, threaded=True)

    # test_360_camera('webcam', source=0, threaded=True, processor={'type': 'equirectangular', 'output_width': 1920, 'output_height': 1080, 'fov': 90})  # noqa: E501
    ...
    # {'capture_type': 'folder', 'source': "media/image_seq", 'sort_by': 'date', 'fps': 30, 'real_time': True, 'loop': True, 'threaded': False},  # noqa: E501
    # {'capture_type': 'screen', 'x': 100, 'y': 100, 'w': 800, 'h': 600, 'fps': 30, 'threaded': True}  # noqa: E501
    # ]
